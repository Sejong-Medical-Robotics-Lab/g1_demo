...
그에 맞는 G1 의 사전 정의 동작을 실행한다.
    ...
    사람이 오른손을 들고 가만히 있음  →  right_hand_up
    사람이 오른손을 흔듦             →  right_hand_wave
    사람이 왼손을 들고 가만히 있음    →  left_hand_up
    사람이 왼손을 흔듦               →  left_hand_wave
    ...
    사람이 팔뚝을 X자로 교차(손이 반대쪽 팔꿈치에)  →  xray (울트라맨 광선)
...
ACTION_LEFT_HAND_UP = 23      # right_hand_up 을 대신 씀 — 아래 설명 참고
...
# 확정: 실기체 SDK 액션 리스트(전체 표)에 왼손 전용 "왼손 들기/왼손 흔들기"
# 액션 자체가 애초에 존재하지 않는다(팀 확인 완료). TODO 아님 — 오른손 ID
# 재사용은 임시 미봉책이 아니라 최종 설계 결정이다. 나중에 SDK가 추가되면
# 그때 새 ID로 교체.
...
HOLD_FRAMES = 3
# 동작을 보낸 뒤 기본으로 이만큼은 새 동작을 받지 않는다.
# 액션마다 실제 소요시간이 다르므로, ACTION_DURATION_S 에 실측값을
# 넣어두면 그 값이 우선 적용된다(없으면 이 기본값 사용).
ACTION_COOLDOWN = 8.0
# 액션 ID별 실제 소요시간(초). 화면 타이머가 실제 로봇 동작 종료 시점과
# 맞게 실측하면서 채워나갈 것. 강제 release가 애니메이션 중간에 안
# 끼어들고 대기열처럼 뒤에 밀려서 실행되는 경우, 여기 값을 실제
# 관찰한 시간(액션 끝~release 실제 반영 시점)으로 맞추는 게 더 정확하다.
ACTION_DURATION_S = {
...
DEBUG_XRAY = False  # 콘솔에 엑스레이 판별 값 실시간 출력 (문제 해결되면 False로)


def is_elbow_cross_pose(lm):
    """한쪽 손목이 반대쪽 팔꿈치에 닿아있으면 엑스레이 자세로 인정.

    실기체 ultraman_ray 애니메이션이 실제로 팔뚝 X자 교차 모양이라 그에
    맞춰 판별한다. 양쪽이 동시에 붙어있을 필요는 없다 — 예를 들어
    오른팔이 어깨 위로 자유롭게 올라가 있어도, 왼손목이 오른쪽
    팔꿈치에 붙어있기만 하면 인정한다. Pose 랜드마크(팔꿈치·손목)만으로
    판별 가능해서 손 모델(Holistic hands) 인식 실패 위험이 없다.

    절대 거리 대신 자기 팔뚝 길이(팔꿈치-손목) 대비 비율로 계산해서
    사람 체구·카메라 거리 차이에 안정적으로 동작한다.
    """
    def ok(i):
...
def classify(lm, left_hand, right_hand, l_state, r_state, now):
    ...
    흔들기/올림. 이 특수 제스처들이 "손 든 것"으로 오판되지 않도록
    먼저 체크한다. 이때 흔들기 상태 기계는 "내려간 것"으로
...
ACTION_MAP = {
    ...
    # 왼손 전용 액션이 SDK에 없어서 오른손 ID로 대신 실행한다 (확정된 설계,
    # TODO 아님 — SDK_API.md 전체 표로 확인 완료).
    "left_hand_up":    (ACTION_RIGHT_HAND_UP, "G1 왼손 올리기 (오른손 액션으로 대신 실행)"),
...
def main():
    ...
    last_action_at = 0.0
    executing = ""
    release_pending = False  # 쿨다운(=화면 타이머)이 0이 되는 순간 강제 정지(release)할지
    ...
    try:
        while True:
            ...
            cooling = (now - last_action_at) < current_cooldown

            # 화면 타이머가 0이 된 순간 — 아직 실제로 동작 중일 수 있으니
            # 강제로 정지(release)시켜서 타이머와 로봇 상태를 맞춘다.
            if release_pending and not cooling:
                release_pending = False
                if arm is not None:
                    try:
                        arm.ExecuteAction(ACTION_RELEASE)
                        print(f"  [{time.strftime('%H:%M:%S')}] 타이머 종료 → 강제 정지(release)")
