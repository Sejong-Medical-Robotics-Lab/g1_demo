#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""probe_odom.py — G1 내장 오도메트리 후보 (토픽, 타입) 자동 탐침
...
CHILD = textwrap.dedent("""
    ...
    signal.alarm(7)                # Read() 가 무한 블록해도 7초면 강제 무수신 처리
    from unitree_sdk2py.core.channel import ChannelFactoryInitialize, ChannelSubscriber
    from {mod} import {cls}

    ChannelFactoryInitialize(0, "{iface}")
    sub = ChannelSubscriber("{topic}", {cls})
    sub.Init()                       # 폴링 모드 (콜백 없음 — 11.x 검증 방식)
    t0 = time.time()
...
def main():
    ...
    print(f"  후보 {len(CANDIDATES)}개, 각 5초씩 격리 시험 (폴링 방식, venv_dds 전용)\n")
    hits = []
