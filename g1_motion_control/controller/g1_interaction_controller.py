...
· 거리 안전 게이트: 사람이 --min-distance(기본 1.0m) 보다 가까우면
  ...
  단, "사람이 아예 안 보이는 경우"(NO PERSON)는 부딪힐 대상이 없으므로
  막지 않는다 — 그래서 카메라 앞에 아무도 없어도 음성 명령은 나간다.
  사람은 보이는데 거리를 못 재면("NO DEPTH") 안전 쪽으로 판단해 막고,
  카메라 프레임이 2초 넘게 끊기면("NO FRAME") 판단 불가라 역시 막는다.
...
# ── 거리 안전 게이트 판단용 시간값 ────────────────────────────────────
# "사람이 안 보임"과 "사람이 있는데 거리를 못 잼"은 전혀 다른 상황이다.
# 전자는 부딪힐 대상이 없으니 통과시키고, 후자만 막아야 한다.
# (예전엔 둘 다 NO DEPTH 로 묶여서, 카메라에 사람이 없으면 음성 명령까지
#  전부 차단됐다.)
PERSON_LOST_HOLD_S = 1.0   # 이 시간 이상 연속으로 안 보여야 "사람 없음" 확정
DEPTH_STALE_S = 1.5        # 마지막 유효 거리값을 이 시간까지만 신뢰
FRAME_STALE_S = 2.0        # 카메라 프레임이 이만큼 끊기면 판단 불가 → 막는다
...
class SharedActionState:
    ...
    def __init__(self, speaker=None, min_distance=None):
        ...
        # 사람 존재 여부. 카메라 루프가 매 프레임 갱신한다.
        self.person_present = False
        self.person_absent_since = time.monotonic()
    ...
    def update_distance(self, distance_m):
        with self.lock:
            ...
            if distance_m is not None:
                self.last_distance_at = time.monotonic()

    def update_person(self, present, center_px):
        """카메라 루프가 매 프레임 부른다. 사람 유무 + 몸통 중심 픽셀 갱신.

        present 는 '이번 프레임에 사람 랜드마크가 잡혔는가'다. center_px 는
        어깨가 안 보이면 사람이 있어도 None 일 수 있으므로, 사람 유무 판단에
        center_px 를 쓰면 안 된다 — 그래서 두 값을 따로 받는다.
        """
        now = time.monotonic()
        with self.lock:
            ...
            if present:
                ...
                if self.person_present or self.person_absent_since is None:
                    # 방금 놓친 시점을 기록. 바로 '없음'으로 확정하지 않는다
                    # (MediaPipe 가 한두 프레임 놓치는 건 흔하다).
    ...
    def safety_status(self):
        """(allowed, reason) 리턴. min_distance 가 None 이면 게이트 자체를 안 쓴다.

        판단 순서
            GATE OFF   게이트 미사용(--no-safety / --no-pose)
            NO FRAME   카메라가 죽음 → 판단 불가라 막는다
            NO PERSON  사람이 없음 → 부딪힐 대상이 없으므로 통과
            NO DEPTH   사람은 있는데 거리를 못 잼 → 막는다
            TOO CLOSE  사람이 너무 가까움 → 막는다
            SAFE       통과
        """
        now = time.monotonic()
        with self.lock:
            ...
            distance_at = self.last_distance_at
            present = self.person_present
            absent_since = self.person_absent_since
            frame_at = self.last_frame_at

        if min_distance is None:
            return True, "GATE OFF"

        if (now - frame_at) > FRAME_STALE_S:
            # 프레임이 안 들어오면 사람이 없는 건지 카메라가 죽은 건지
            # 알 수 없다. 안전 쪽으로 막는다.
            return False, "NO FRAME"

        if not present and absent_since is not None \
                and (now - absent_since) >= PERSON_LOST_HOLD_S:
            return True, "NO PERSON"

        if distance_m is None or (now - distance_at) > DEPTH_STALE_S:
    ...
    def distance_for_display(self):
        """화면에 띄울 거리. 오래된 값이면 None(=--)으로 본다."""
        now = time.monotonic()
        with self.lock:
            if self.last_distance is None or (now - self.last_distance_at) > DEPTH_STALE_S:
                return None
...
def depth_loop(args, state, stop_event):
    ...
    while not stop_event.is_set():
        ...
        # center_px 가 None 이면 아무것도 쓰지 않는다. 예전엔 여기서
        # update_distance(None) 을 해서, 사람이 프레임에서 사라지는 순간
        # 곧바로 NO DEPTH 가 되어 음성 명령까지 막혔다. 이제는 값을
        # 그대로 두고 DEPTH_STALE_S 로 자연히 만료시킨다.
...
def main():
    ...
    ap.add_argument("--speech-pad", type=float, default=0.3,
                    help="음성 파형 뒤에 붙일 무음 길이(초). 마지막 음절이 "
                         "잘리면 늘리세요 (기본 0.3)")
    ap.add_argument("--speech-lead", type=float, default=0.2,
                    help="실시간보다 이만큼 앞서서 오디오를 밀어넣는다(초). "
                         "말이 통째로 잘리면 줄이고, 중간에 끊기면 늘리세요")
    ap.add_argument("--speech-chunk", type=int, default=0,
                    help="0이면 쪼개지 않고 한 번에 전송(기본). 32000 = 1초치씩")
    ap.add_argument("--speech-no-stop", action="store_true",
                    help="재생 후 PlayStop 을 부르지 않는다 (꼬리 잘림 진단용)")
    ap.add_argument("--speech-debug", action="store_true",
                    help="청크 전송 타이밍을 콘솔에 찍는다")
    ...
    speaker = Speaker(audio_client, SPEECH_MAP, wav_dir=SPEECH_DIR,
                      tail_pad_s=args.speech_pad,
                      lead_s=args.speech_lead,
                      chunk_bytes=args.speech_chunk,
                      call_stop=not args.speech_no_stop,
    ...
    if safety_active:
        ...
        print("           사람이 안 보이면(NO PERSON) 막지 않는다 — "
              "음성 명령은 사람 없이도 실행된다")
    ...
    if not args.no_pose:
        ...
        try:
            ...
            while True:
                ...
                if safety_active:
                    state.update_person(res.pose_landmarks is not None, center_px)
                ...
                if not args.no_window:
                    ...
                    if safety_active:
                        ...
                        dist_val = state.distance_for_display()
